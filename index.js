$(document).ready(function(){
                    ...
                    + '<div class="card-body">' 
                        + '<div class="row">' 
                            + '<div class="row">' 
                                ...
                                + '</svg>' 
                                + '<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-x-square close-style" id="delete-task" fill="currentColor" xmlns="http://www.w3.org/2000/svg">'
                            ...
                            + '<div class="col-5">' 
                                + '<div class="row">' 
                                    + '<span class="stopwatch-style" id="stopwatch">' 
                                        + '('+ durationSplit[0] + ':' + durationSplit[1] + ':' + durationSplit[2] + ')'
                                    + '</span>'
                                + '</div>'
                                + '<div class="row">'
                                    + '<div class="col">' 
                                        + '<button type="button" class="btn btn-secondary start-style" id="start">Start</button>'
                                    + '</div>' 
                                    + '<div class="col">' 
                                        + '<button type="button" class="btn btn-secondary end-style" id="end" disabled="disabled">End</button>'
                                    + '</div>' 
                                + '</div>' 
                            + '</div>' 
                        + '</div>'
    ...
    $(document).on('click', '#edit-task', function(){
        ...
        console.log("[EDIT] taskName: " + taskName)
        var index = 0
        for(i = 0; i < taskDetails.length; i++) {
            if(taskDetails[i].name == taskName) {
                index = i
                $(this).closest("li").find('.text-style').empty()
                $(this).closest("li").find('.text-style').append(
                    '<div class="row" id="save-function">' 
                        + '<div class="row" id="text-area-style">'
                            + '<textarea class="form-control" id="text-to-be-saved" rows="1"></textarea>'
                        + '</div>'
                        + '<div class="row" id="save-style">'
                            + '<button type="submit" class="btn btn-warning mb-3 save-style" id="edit-save">Save</button>'
                        + '</div>'
                    + '</div>'
                );
            }
        ...
        $(document).on('click', "#edit-save", function(){
            // Saving to array
            var newTask = $(this).closest("#save-function").find('#text-to-be-saved').val();
            console.log(newTask)
            taskDetails[index].name = newTask 

            // Printing
            console.log($(this).closest("#save-function"))
            $(this).closest("#save-function").replaceWith(
                '<span class="p-margin">' 
    ...
    $(document).on('click', '#delete-task', function(){
        ...
        for(i = 0; i < taskDetails.length; i++) {
            console.log(taskDetails[i].duration)
